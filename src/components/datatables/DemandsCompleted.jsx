...
export default function DemandsCompletedTable(props) { 
  ...
  //
    ...
    const columns = [
        ...
        { name: 'number', label: "Número"  , options: {
            ...
            const demand = props.completedDemands[tableMeta.rowIndex]
            var number = ""
    ...
    const seeDemand = (demandId) => {
        ...
        setSelectedDemand(props.completedDemands.find(demand => demand.id === demandId));
    }

    const updateDemand = (demandId) => {
        const demandFound = props.completedDemands.find(demand => demand.id === demandId);
