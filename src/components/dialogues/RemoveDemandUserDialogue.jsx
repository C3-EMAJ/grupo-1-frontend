...
import { deleteDemandDocument, removeUserOnDemand } from '../../data/axios/apiCalls';
...
export default function RemoveDemandUserDialogue(props) {
  ...
  // Para excluir um usuário da demanda:
  const handleDeleteDemandDocument = (e) => {
    e.preventDefault();

    removeUserOnDemand({"idUser": props.userToRemove.id, "idDemand": props.demandSelected.id})
        .then(response => {
            // Se der certo:
            if (response.status == 200) {
                ...
                props.handleAlertMessage("success", "Usuário removido com sucesso.");
                props.setDemandSelected(false)
                props.setShowEditUsers(false);
            }
            else {
                props.setIsLoading(false);
                handleAlertMessage("error", "Erro ao remover o usuário.");
            ...
            <div className=" flex sm:items-start mt-5">
                ...
                <div className="text-left">
                    <p className="text-sm ml-3 text-gray-500 text-justify">
                        Você deseja remover o usuário <span style={{ color: 'red', fontWeight: '700' }}>{props.userToRemove.name}</span> dessa demanda? Você pode adicioná-lo em outro momento.
                    </p>
                </div>
            </div>
        </div>
